    ...
    <footer className="border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950/60 transition-colors">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8 lg:gap-8">
            ...
            <div className="pt-2 flex flex-col gap-2.5 text-xs">
              ...
              <div>
                <a
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-orange-50 dark:bg-orange-950/30 text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-800/50 hover:bg-orange-100 dark:hover:bg-orange-900/40 font-semibold text-xs transition-colors"
                >
          ...
          {/* Converters Column (2-Column Grid on large screens) */}
          <div>
            ...
            <ul className="space-y-2">
              {converters.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="text-xs text-zinc-600 dark:text-zinc-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors inline-flex items-center justify-between w-full gap-1 group py-0.5"
                  >
                    <span className="truncate">{item.name}</span>
                    {item.badge && (
                      <span className="text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.2 rounded bg-emerald-100/80 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 shrink-0">
                        {item.badge}
                      </span>
                    )}
                  </Link>
        ...
        <div className="border-t border-zinc-200 dark:border-zinc-800/80 mt-12 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-zinc-500 dark:text-zinc-400">
