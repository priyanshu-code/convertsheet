...
export interface SplitXmlInputProps {
...
<records>
  <item>
    <id>1</id>
    <name>Analytics Cloud Engine</name>
  ...
  <item>
    <id>2</id>
    <name>Realtime Data Pipeline</name>
...
export function SplitXmlInput({
  ...
  const acceptAttribute = [
    ...acceptedExtensions,
    ...(config.acceptedMimeTypes || ["application/xml", "text/xml", "text/plain"]),
  ].join(",");

  const stats = useMemo(() => {
    const trimmed = text.trim();
    if (!trimmed) {
      return { isEmpty: true, lineCount: 0, charCount: 0 };
    }
  ...
  const handleParseText = useCallback(
    (contentToParse?: string) => {
      ...
      const file = new File([trimmed], "input.xml", { type: "application/xml" });
      onFileSelect(file);
    },
  ...
  const handleFormat = useCallback(() => {
    ...
    try {
      ...
      let pad = 0;
      const regex = /(>)(<)(\/*)/g;
      const xml = text.replace(regex, "$1\r\n$2$3");
      xml.split("\r\n").forEach((node) => {
        let indent = 0;
        if (node.match(/.+<\/\w[^>]*>$/)) {
          indent = 0;
  ...
  return (
    ...
    >
      {/* Left Panel: Monospace XML Textarea with Quick Actions */}
      <div className="flex flex-col rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/40 p-4 sm:p-5 transition-all">
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-zinc-200/60 dark:border-zinc-800/60">
          <div className="flex items-center gap-2 text-[11px] text-zinc-500 dark:text-zinc-400">
            {stats.isEmpty ? (
              <span>Paste valid XML markup with repeating record nodes</span>
            ) : (
              <span className="flex items-center gap-1.5 font-mono">
                <span className="text-emerald-600 dark:text-emerald-400 font-semibold">
                  {stats.lineCount} lines detected
                </span>
                <span>•</span>
      ...
      <div className="flex flex-col">
        ...
        <div
          data-testid="split-xml-dropzone-area"
          role="button"
